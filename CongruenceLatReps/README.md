...
L₁, ..., L₃₅ in the article.  This file holds the finite
unary algebras Bᵢ with Con(Bᵢ) isomorphic to Lᵢ, 29 algebras in all:

    B1 B2 B3 B4 B4-prime B5 B6 B7 B8 B9 B12 B13 B15 B17 B19 B21 B23 B24
    B25 B26 B27 B28 B29 B30 B31 B32 B33 B34 B35

The base set of Bᵢ is {0, 1, ..., n-1} and each operation is unary,
given by its table of values.  Algebras of cardinality less than 11, and also
B₃₃, are known to be of minimum size among algebras with that
congruence lattice.

B4-prime is a second representation of L₄, this one by an intransitive group
action.  There is no entry for L₁₀, which is still not known to be the congruence
lattice of any finite algebra, nor for L₁₁, L₁₄, L₁₆, L₁₈, L₂₀, and L₂₂, for which
the article gives no explicit small algebra.  See the article for what is known in
each of those cases.
...
Before September 2026 the copy of this file in this repository had an incorrect
B₂₈: it had six operations rather than seven, and its congruence lattice had 8
elements, so it was not L₂₈.  This was [issue #20][] of the fin-lat-rep repository
and was fixed there in [PR #22][].  Anyone who took B₂₈ from release v1.0.1 of
this repository, or from the master branch before September 2026, has the wrong
algebra.
...
+  `RegActSmallGroup216-153.ua`, the regular action of `SmallGroup(216,153) =`
   `((C3 x C3) : Q8) : C3`, in whose subgroup lattice L₁₁ and L₂₀ appear as the
...
+  `A4xA4_sdp_C2.ua`, the group `SmallGroup(288,1025) =` `(A4 x A4) : C2` as a
   ...
   L₁₇ is an interval.  This one is a group, not a G-set: loading it gives you 288
   elements and one operation of arity 2;
